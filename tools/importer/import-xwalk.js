...
/* AUTO-DERIVED from import-help-and-support.js (do not hand-edit; run
 * tools/importer/.make-xwalk-import.py to regenerate). Same parsers,
 * transformers, and DAM-asset rewrites, but OUTPUT_MODE xwalk lands the
 * page under /content/TAQA for the AEM author (xwalk) import. */
...
export default {
  transform: (payload) => {
    ...
    const deepPath = new URL(pageUrl).pathname
      ...
      .replace(/\.html?$/, '');
    let path;
    if (OUTPUT_MODE === 'xwalk') {
      // Land the page under the AEM site path. deepPath is the source pathname
      // (e.g. /addc/en-us/residential/help-and-support/transfer-and-removal-of-
      // electricity-services); prefix with the site root so md2jcr writes it to
      // /content/TAQA/... in the author instance.
      path = WebImporter.FileUtils.sanitizePath(`/content/TAQA${deepPath}`);
    } else if (OUTPUT_MODE === 'deep') {
      path = WebImporter.FileUtils.sanitizePath(deepPath);
    } else {
      path = WebImporter.FileUtils.sanitizePath('/index');
    }
